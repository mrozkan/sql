...
SELECT 
	customer_id,
	market_date,
	DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date) AS visit_number_by_date
FROM (
...
SELECT 
  cp.*, 
  ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY market_date DESC, transaction_time DESC) AS rn_desc
...
SELECT *
FROM product
WHERE product_size IS NOT NULL AND product_size GLOB '*[0-9]*';
...
SELECT market_date, total_sales, 'best_day' AS tag FROM ranked WHERE rank_desc = 1
UNION
SELECT market_date, total_sales, 'worst_day' AS tag FROM ranked WHERE rank_asc = 1;
...
SELECT 
  vp.vendor_name,
  vp.product_name,
  vp.original_price * 5 * nc.n AS potential_revenue_if_5_each_customer
...
VALUES (9999, 'Emre Pie', '1 slice', 1, 'unit', CURRENT_TIMESTAMP);
...
WHERE product_id = 9999
  AND snapshot_timestamp < (
    SELECT MAX(snapshot_timestamp) FROM product_units WHERE product_id = 9999
  );
